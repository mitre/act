...
This guide explains how to customize the ACT website's theme, colors, and spacing using the ACT Design System layer.

## Color Theme

### Primary Color

The site uses blue as the primary color, configured in the layer's `app.config.ts`:
...
Dark mode is set as default in the main app's `nuxt.config.ts`:
...
Spacing is controlled via CSS overrides in `layers/act-design-system/assets/css/design-system.css`:
...
Adjust container sizes in the design system CSS:
...
Mobile-specific spacing is handled in the design system:
...
The design system layer uses Nuxt UI components with careful overrides:
...
You can override design system defaults in your main app's `app.config.ts`:
...
export default defineAppConfig({
  ui: {
    ...
    pageSection: {
      wrapper: 'py-4 sm:py-6 lg:py-8' // Even tighter spacing
    }
  }
})
```
...
Add custom CSS variables in your main app's CSS:

```css
:root {
  --custom-spacing: 0.5rem;
  --custom-container: 60rem;
}
```
